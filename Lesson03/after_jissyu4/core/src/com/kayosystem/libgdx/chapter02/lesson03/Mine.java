...
class Mine {
    ...
    public void update(float deltaTime) {
        ...
        if (!hasCollided) {
            // 障害物のアニメーション
            visiblePart = 0.2f + showFunc(timeSinceCreation * 0.2f +
                    phaseShift) * 0.4f;
            collisionCircle.y = origin.y - origin.height / 2 +
                    visiblePart * origin.height;
        }
    }

    private float showFunc(float t) {
        t = t % 1.0f;
        return Math.min(Math.max(t * (-12.5f * t + 12.5f) - 2.0f, 0.0f),
                1.0f);
    }

    // 描画
    public void draw(SpriteBatch batch) {
        region.setRegion(0, 0, region.getTexture().getWidth(),
                (int) (region.getTexture().getHeight() * visiblePart));
