package com.kayosystem.libgdx.chapter02.lesson03;
...
public class MyGdxGame extends ApplicationAdapter {
    ...
    public void create() {
        ...
        camera.position.set(camera.viewportWidth / 2, camera.
                viewportHeight / 2, 0);
        ...
        // ゴール地点の決定
        finishX = (background.stageWidth - VIEWPORT_WIDTH) / Background.
                SPEED + Hero.HERO_LEFT_X;
    ...
    private void updateWorld() {
        ...
        if (gameState == GameState.GameOver || gameState == GameState.
                GameCleared) {
            return;
        }
        // 衝突判定
        ...
        for (Chip chip : generator.chips) {
            if (!chip.isCollected && Intersector.overlaps(chip.
                    collisionCircle, heroCollision)) {
                chip.collect();
                coinSound.play();
                score += chip.getScore();
            }
        ...
        for (Mine mine : generator.mines) {
            if (!mine.hasCollided && Intersector.overlaps(mine.
                    collisionCircle, heroCollision)) {
                mine.collide();
                collisionSound.play();
                hero.die();
